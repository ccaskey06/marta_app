...
# WORKS
@app.route('/register-old', methods=['POST', 'GET'])
def register():
	...
	if request.method == 'POST':
		...
		try:
			...
			db_cursor.execute("""INSERT INTO `User`(`Username`, `Password`, `IsAdmin`) VALUES (%s, %s, 0)""", [username, hash_pass])

		except (Exception): # query fails if username (primary key) already exists
		...
		try:
			...
			db_cursor.execute("INSERT INTO `Passenger`(`Username`, `Email`) VALUES (%s, %s)", [username, email])

		except (Exception): # query fails if email (unique) already exists
		...
		if breezecard == "":
			...
			generate_breezecard(db_cursor, username)
		...
		## for using "Have Breezecard", check if Breezecard is already in the system
			...
			try:
				...
				db_cursor.execute("""INSERT INTO `Breezecard`(`BreezecardNum`, `Value`, `BelongsTo`) VALUES (%s, 0, %s)""", [breezecard, username])

			except (Exception): # breezecard already exists in database - query fails

				count_null = None

				try:
					# will either return 0 or 1 tuple(s)
					db_cursor.execute("""SELECT * FROM `Breezecard` WHERE `BreezecardNum` = %s AND `BelongsTo` IS NULL""", [breezecard])

					count_null = db_cursor.rowcount # number of tuples

				except (Exception): # query fails for some reason - generate random card and go to passenger dashboard

					generate_breezecard(db_cursor, username)
					return redirect(url_for('pass_dashboard', username=username))

				# only works if above query worked
				if count_null > 0: # this breezecard has a NULL owner
					try:
						# card exists but doesn't belong to anyone (i.e. BelongsTo = NULL)
						db_cursor.execute("""UPDATE `Breezecard` SET `BelongsTo` = %s WHERE `BreezecardNum` = %s""", [username, breezecard])
					...
					try:
						...
						db_cursor.execute("""INSERT INTO `Conflict`(`Username`, `BreezecardNum`, `DateTime`) VALUES (%s, %s, %s)""", [username, breezecard, date_time])
						generate_breezecard(db_cursor, username) # generate new breezecard for username

					except (Exception): # query fails - there already exists a conflict for this username and breezecard; generate new breezecard and redirect to passenger dashboard
...
def generate_breezecard(db_cursor, username):
	...
	while(z):
		...
		try:
			...
			db_cursor.execute("""INSERT INTO `Breezecard`(`BreezecardNum`, `Value`, `BelongsTo`) VALUES (%s, 0, %s)""", [card_number, username])
...
# 					dbCursor.execute("""UPDATE `Breezecard` SET `BelongsTo` = %s, `Value` = 0 WHERE `BreezecardNum` = %s""", [username, cardNumber])
